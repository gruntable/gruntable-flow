...
export default function useOrchestrator({
  ...
  const pollRunStatus = useCallback(async (signal) => {
    ...
    if (status.status === 'node_done') {
      ...
      console.log('  pauseForReview:', currentNode?.settings?.processing?.pauseForReview);
      console.log('  node_response from status:', status.node_response);
      console.log('  node_response type:', typeof status.node_response);
      console.log('  node_response JSON:', JSON.stringify(status.node_response));

      // Capture node response for display in Run Tab
      // Only update if content actually changed to prevent infinite re-render loops
      ...
      console.log('[WF-POLL] Checking node_response - truthy?', !!newResponse, '| contentChanged?', hasContentChanged);
      if (newResponse && hasContentChanged) {
        console.log('[WF-POLL] Setting pauseReviewMessage to:', newResponse);
        setPauseReviewMessage(newResponse);
      } else if (!newResponse) {
        console.log('[WF-POLL] node_response is falsy - NOT setting pauseReviewMessage');
      } else {
        console.log('[WF-POLL] node_response unchanged - skipping update');
      }

      const shouldPauseForReview = currentNode?.settings?.processing?.pauseForReview !== false;

      if (NODE_REGISTRY[currentNode?.node_type]?.is_export) {
        console.log('[WF-POLL] Export node - skipping Pause Review, will wait for export_downloaded');
        setPauseReviewTableName(null);
      } else if (currentNode?.table_name && status.output_table) {
        console.log('[WF-POLL] Setting pauseReviewTableName:', currentNode.table_name);
        setPauseReviewTableName(currentNode.table_name);
        setTableRefreshKey(k => k + 1);

        if (!shouldPauseForReview) {
          console.log('[WF-POLL] pauseForReview is false - auto-advancing immediately...');
        ...
        console.log('[WF-POLL] No table output (node_response only) - skipping table fetch, auto-advancing...');
        const totalNodes = runConfigRef.current?.nodes?.length ?? 0;
        const isSingleNodeRun = totalNodes === 1;
        if (isSingleNodeRun) {
          advanceRun(runGuidRef.current).then(response => {
            console.log('[WF-POLL] Auto-advance (no table) response:', response);
            setOrchestratorStatus(response.status);
            setCurrentNodeIndex(response.current_node_index);
          }).catch(err => {
    ...
    const triggerAutoNode = async () => {
      try {
        console.log('[Webhook] Sending webhook request:', {
          ...
          url: currentNode.webhook_url,
          body: currentNode.webhook_body
        });
        ...
        console.log('[Webhook] Webhook response status:', response.status);
        ...
        if (NODE_REGISTRY[currentNode.node_type]?.is_export) {
          ...
          console.log('[Webhook] Webhook response data:', responseData);
        }
      } catch (err) {
