...
export const convertNodesToOrchestratorFormat = (nodes, tableNames, nodePrompts, nodeSourceTables, allTables = []) => {
  const result = nodes.map((node, index) => {
    ...
    // Multi-input source tables (e.g. ai_transformation, code)
    let sourceTableNames = null;
    if (manifest?.requires_table_input && (manifest?.max_input_tables ?? 1) > 1) {
      const stored = nodeSourceTables[node.id];
      console.log(`[WF-META] Multi-input node "${node.label}" - nodeSourceTables[${node.id}]:`, stored);
    ...
    // Single source table (e.g. export_excel, ai_export)
    ...
    if (manifest?.requires_table_input && (manifest?.max_input_tables ?? 1) === 1) {
      ...
      console.log(`[WF-META] Single-input node "${node.label}" - sourceTableName:`, sourceTableName);
    }
    ...
    return {
      ...
      fileName: isExport
        ? (node.settings?.processing?.file_name || defaultFileName)
        : undefined,
