...
export default function TemplateCard({ name, platform, inputMethod, description, showChips = true, imgStyle = false, img_url = null, onClick }) {
    ...
    return (
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: C.black, lineHeight: 1.35 }}>{name}</div>
          {description && (
            <div style={{ fontSize: 11, color: C.muted, lineHeight: 1.5 }}>{description}</div>
          )}
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 14, fontWeight: 600, color: C.black, lineHeight: 1.4 }}>
        {name}
      </div>
