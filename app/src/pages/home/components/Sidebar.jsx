...
export default function Sidebar({ activePage = 'home' }) {
  ...
  useEffect(() => {
    ...
    function handleMouseDown(e) {
      const ref = openPopover === 'templates'  ? templatesRef
                : openPopover === 'references' ? referencesRef
                : profileRef;
    ...
    <div style={{
      ...
      <div style={{ padding: collapsed ? '14px 6px' : '14px 12px', display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          {!collapsed && (
            <span style={{ fontWeight: 700, fontSize: 15, color: C.black, flex: 1, whiteSpace: 'nowrap' }}>
              Gruntable
            </span>
          )}
          <button
        ...
        {/* Tables nav item (disabled) */}
        <div style={navItemStyle(activePage === 'tables', collapsed, true)}>
          <span style={{ fontSize: 14, flexShrink: 0 }}>📋</span>
      ...
      <div style={{ padding: collapsed ? '14px 6px' : '14px 12px', display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <div style={{ position: 'relative' }} ref={templatesRef}>
          <div
            onClick={() => setOpenPopover(v => v === 'templates' ? null : 'templates')}
            style={navItemStyle(false, collapsed)}
            onMouseEnter={e => { if (openPopover !== 'templates') e.currentTarget.style.background = C.bg; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
          >
            <span style={{ fontSize: 14, flexShrink: 0 }}>📄</span>
            {!collapsed && <span>Templates ↗</span>}
          </div>
          {openPopover === 'templates' && <CommunityPopover links={COMMUNITY_LINKS} />}
        </div>
