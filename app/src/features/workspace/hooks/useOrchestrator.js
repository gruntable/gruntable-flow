...
export default function useOrchestrator({
  ...
  const pollRunStatus = useCallback(async (signal) => {
    ...
    const prevNodeIndex = currentNodeIndex;
    const prevStatus = orchestratorStatus;
    ...
    if (status.status === 'node_done') {
      ...
      if (currentNode?.node_type === 'export_excel') {
  ...
  }, [stopPolling, currentNodeIndex, orchestratorStatus]);
  ...
  useEffect(() => {
    ...
    const triggerAutoNode = async () => {
      ...
      console.log('  URL:', currentNode.webhook_url);
  ...
  const startRunWorkflow = async () => {
    ...
    try {
      ...
      console.log('[WF-DEBUG] Sending to orchestrator /start:', JSON.stringify({ trigger_mode: 'run_workflow', nodes: orchestratorNodes }, null, 2));
      const response = await startRun('run_workflow', orchestratorNodes);
  ...
  const startSingleNode = async (nodeId) => {
    ...
    try {
      ...
      console.log('[WF-DEBUG] Sending to orchestrator /start (single_node):', JSON.stringify({ trigger_mode: 'single_node', nodes: orchestratorNodes }, null, 2));
      const response = await startRun('single_node', orchestratorNodes);
